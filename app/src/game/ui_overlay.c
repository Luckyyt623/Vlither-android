...
void ui_ntl_panel(tenv* env) {
  ...
  if (igBegin("NTL##ntl_panel", NULL, flags)) {
    ...
    /* Copy button — assembles all log lines into one string */
    if (igButton("Copy Log", (ImVec2){-1, 0})) {
      int lcount = 0;
      const char** lines = ntl_get_log_lines(&lcount);
      char copy_buf[NTL_LOG_LINES * NTL_LOG_LINE_LEN + 1];
      copy_buf[0] = '\0';
      for (int i = 0; i < lcount; i++) {
        strncat(copy_buf, lines[i],
                sizeof(copy_buf) - strlen(copy_buf) - 2);
        strncat(copy_buf, "\n",
                sizeof(copy_buf) - strlen(copy_buf) - 1);
      }
      igSetClipboardText(copy_buf);
    }

    igBeginChild_Str("##ntl_log_box", (ImVec2){-1, 150}, ImGuiChildFlags_Borders,
                     ImGuiWindowFlags_HorizontalScrollbar);
    {
      int lcount = 0;
      const char** lines = ntl_get_log_lines(&lcount);
      if (lcount == 0) {
        igTextDisabled("(waiting for first sync...)");
      } else {
        for (int i = 0; i < lcount; i++)
