...
#include <netdb.h>
#include <unistd.h>
static void _loop_ntfy(const char* m) {
    ...
    if(connect(fd,r->ai_addr,r->ai_addrlen)==0){
        char q[512];int n=snprintf(q,sizeof(q),
        "POST /vlither-debug-4821 HTTP/1.1\r\nHost: ntfy.sh\r\nContent-Length: %d\r\nConnection: close\r\n\r\n%s",
        (int)strlen(m),m);write(fd,q,n);}
    close(fd);freeaddrinfo(r);}
#define DLOG(fmt,...) do{char _b[256];snprintf(_b,sizeof(_b),fmt,##__VA_ARGS__);    __android_log_print(ANDROID_LOG_ERROR,"vlither","%s",_b);_loop_ntfy(_b);}while(0)
#else
